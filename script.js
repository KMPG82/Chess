...
console.log(squares);
function alternateSquareColors() {
    let row = 0
    for (let i = 0; i < squares.length; i++) {
        row = Math.floor(64 + i / 8)
        if (row % 2 === 0) {
            if (squares[i].id % 2 === 0) {
                squares[i].classList.add('brownSquare')
            }
            else {
                squares[i].classList.add('blackSquare')
            }
        }
        else {
            if (squares[i].id % 2 === 0) {
...
alternateSquareColors()

function addPieces() {
    for (let i = 0; i < squares.length; i++) {
        squares[i].innerHTML = gameStart[i];
    }
}
addPieces();
...
function setPieceColors() {
    for (let i = 0; i < gamePieces.length; i++) {
        if (i < 16) {
            gamePieces[i].classList.add('blackPiece')
        }
        else {
            gamePieces[i].classList.add('whitePiece')
